...
namespace UnityCode.Core.Scripts.ObjectPooling.Editor
{
    [CustomEditor( typeof( ObjectPool ) )]
    public class PooolingEditor : UnityEditor.Editor
    {
        /// <summary>
        ...
        private ObjectPool _objectPoolTarget;
        ...
        {
            _objectPoolTarget = target as ObjectPool;
            _objectPoolTarget.recycleBinCollection = (target as ObjectPool).recycleBinCollection;

            _prefabFoldouts = new List<bool>();
            if( _objectPoolTarget.recycleBinCollection != null )
                for( int n = 0; n < _objectPoolTarget.recycleBinCollection.Count; n++ )
        ...
        private void clearNullReferences()
        {
            ...
            int n = 0;
            while( n < _objectPoolTarget.recycleBinCollection.Count )
            {
                if( _objectPoolTarget.recycleBinCollection[n].prefab == null )
                    _objectPoolTarget.recycleBinCollection.RemoveAt( _objectPoolTarget.recycleBinCollection.Count - 1 );
                else
                    n++;
            }
        }
        ...
        /// <summary>
        /// adds a new bin to the ObjectPool collection
        /// </summary>
        /// <param name="go">Go.</param>
        private void addRecycleBin( GameObject go )
        {
            if( _objectPoolTarget.recycleBinCollection == null )
                _objectPoolTarget.recycleBinCollection = new List<ObjectRecycleBin>();

            if( _objectPoolTarget.recycleBinCollection != null )
            {
                foreach( var recycleBin in _objectPoolTarget.recycleBinCollection )
                {
            ...
            var newPrefabPool = new ObjectRecycleBin();
            newPrefabPool.prefab = go;

            _objectPoolTarget.recycleBinCollection.Add( newPrefabPool );
            while( _objectPoolTarget.recycleBinCollection.Count > _prefabFoldouts.Count )
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                if( _prefabFoldouts.Count < _objectPoolTarget.recycleBinCollection.Count )
                {
                    for( var i = 0; i < _objectPoolTarget.recycleBinCollection.Count - _prefabFoldouts.Count; i++ )
                        _prefabFoldouts.Add( false );
                }
                //base.OnInspectorGUI();
                //return;
            ...
            for( int n = 0; n < _objectPoolTarget.recycleBinCollection.Count; n++ )
            {
                var prefabPool = _objectPoolTarget.recycleBinCollection[n];
                ...
                if( GUILayout.Button( "X", buttonStyle, GUILayout.Width( 20f ), GUILayout.Height( 15f ) ) && EditorUtility.DisplayDialog( "Remove Recycle Bin", "Are you sure you want to remove this recycle bin?", "Yes", "Cancel" ) )
                    _objectPoolTarget.recycleBinCollection.RemoveAt( n );
                ...
                if( _prefabFoldouts[n] )
                {
                    ...
                    if( _objectPoolTarget.persistBetweenScenes )
                    {
                        EditorGUILayout.BeginHorizontal();
        ...
        private void dropAreaGUI()
        {
            ...
            GUI.Box( dropArea, "Drop a Prefab or GameObject here to create a new can in your ObjectPool", boxStyle );
