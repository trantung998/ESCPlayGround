...
//            this.StartCoroutine(this.ListenForEmitDespawn(emitter));
            Timing.RunCoroutine(ListenForEmitDespawn(emitter));
            return emitter;
        }
...
//            this.StartCoroutine(this.DoDespawnAfterSeconds(instance, seconds, false, null));
            Timing.RunCoroutine(DoDespawnAfterSeconds(instance, seconds, false, null));
        }
...
//            this.StartCoroutine(this.DoDespawnAfterSeconds(instance, seconds, true, parent));
            Timing.RunCoroutine(DoDespawnAfterSeconds(instance, seconds, true, parent));
        }
        ...
        /// </summary>
        private IEnumerator<float> DoDespawnAfterSeconds(Transform instance, float seconds, bool useParent,
            Transform parent)
        {
...
//        }
        ...
        private IEnumerator<float> ListenForEmitDespawn(ParticleSystem emitter)
        {
            ...
            yield return Timing.WaitForSeconds(emitter.main.startDelayMultiplier + 0.25f);
...
//                this.spawnPool.StartCoroutine(CullDespawned());
                Timing.RunCoroutine(CullDespawned());
            }
            return true;
        }
        ...
        /// </summary>
        internal IEnumerator<float> CullDespawned()
        {
            if (this.logMessages)
            ...
            yield return Timing.WaitForSeconds(this.cullDelay);
